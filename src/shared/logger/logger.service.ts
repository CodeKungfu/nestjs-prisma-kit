...
import { DEFAULT_ERROR_LOG_NAME, DEFAULT_MAX_SIZE, DEFAULT_WEB_LOG_NAME, LOGGER_MODULE_OPTIONS, PROJECT_LOG_DIR_NAME } from './logger.constants';
...
export class LoggerService implements NestLoggerService {
  ...
  protected recordMessages(messages: unknown[], context = '', logLevel: WinstonLogLevel = 'info', stack?: string) {
    messages.forEach((message) => {
      const output = isPlainObject(message) ? JSON.stringify(message, (_, value) => (typeof value === 'bigint' ? value.toString() : value), 0) : (message as string);
  ...
  protected printMessages(messages: unknown[], context = '', logLevel: WinstonLogLevel = 'info', writeStreamType?: 'stdout' | 'stderr') {
  ...
  private updateAndGetTimestampDiff(): string {
    ...
    const result = includeTimestamp ? yellow(` +${Date.now() - LoggerService.lastTimestampAt}ms`) : '';
